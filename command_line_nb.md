...
9-26-16
LUMPY:
Quinlan lab finally back from conference. Spoke with Brent and he suggested just downloading lumpy through bioconda (associated with miniconda for python), this would allow me to have my own python library for the necessary modules.

Downloading and setting up miniconda:

From http://conda.pydata.org/miniconda.html downloaded the Python 2.7 64-bit bash installer, transfered to my cluster home directory using Globus and ran with:

$bash Miniconda2-latest-Linux-x86_64.sh

Did not work at first, though .bashrc was appended with anaconda library. Was seeing an "Unexpected end file error" when I opened the terminal, so I copied the bashrc script to my home directory with:

$cp .bashrc bashrc_backup.txt

and commented out some if statements that were added for the local perl library Barry set up for VAAST. With this Conda stated working so I ran:
...
this installed bioconda and lumpy through bioconda. I did a test run of lumpy with just:

$lumpyexpress

Recieved exceptions about missing packages. Used conda to install necessary packages.
...
Ran lumpyexpress again, only exception received was a missing -B agrument (no bam input) thus it seems to be good to go. Ran sample 15-0025869 as it had a few large regions of homozygosity on Omicia Opal.

$lumpyexpress -B /scratch/ucgd/lustre/work/u0695383/gnomex/15-12-01_Nant-Kardon-CDH/UGP/Data/PolishedBams/15-0025869.bam -o /scratch/ucgd/lustre/work/u0695383/15-0025869.bam.lumpy.vcf

Ran on a seperate screen, appeared to take more than 4 hours. Output file, 15-0025869.bam.lumpy.vcf, is 4.1Mb, which is promising since we do not expect a lot of SVs in our samples. On first look through there are calls on each chromosome, so I think we definitely need to apply some filters. Will read through the original Lumpy publication and talk to Ryan and Aaron about this. Will start running the other connective tissue samples using the same parameters.
